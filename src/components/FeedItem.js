...
export default async function FeedItem(params) {
  ...
  async function getData() {
    ...
    const photoInfoData = await getPhotoInfo(params.photo.id, params.photo.secret);
          ...
          <div className="feed-item-info-container d-flex p-2">
            <div className="feed-item-info w-50">
              <div className="feed-item-author">
                by
                <strong>
                  {userInfo.data.profile.first_name ? ` ${userInfo.data.profile.first_name}` : ''}
                  {userInfo.data.profile.last_name ? ` ${userInfo.data.profile.last_name}` : ''}
                  {!userInfo.data.profile.first_name && !userInfo.data.profile.last_name
                    ? ` User ${params.photo.owner}`
                    : ''}
                </strong>
              </div>
              <div className="feed-item-date">
                Taken {formatDate(new Date(parseInt(photoInfo.data.photo.dates.lastupdate) * 1000))}
              </div>
            </div>
